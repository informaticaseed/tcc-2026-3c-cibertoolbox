...
=======
        lateral = ctk.CTkFrame(self, width=240, corner_radius=0, fg_color=SIDE, border_width=1, border_color=BORDER)
        lateral.grid(row=0, column=0, sticky="nsew")
        lateral.grid_propagate(False)
        ctk.CTkLabel(lateral, text="CONFIGURAÇÕES", font=("Consolas", 21, "bold"), text_color=ORANGE).pack(anchor="w", padx=24, pady=(30, 5))
        ctk.CTkLabel(lateral, text="Preferências do CiberToolBox", text_color=MUTED).pack(anchor="w", padx=24)
        ctk.CTkLabel(lateral, text="As alterações são salvas em JSON local e podem ser restauradas a qualquer momento.", wraplength=190, justify="left", text_color=MUTED).pack(anchor="w", padx=24, pady=30)
        ctk.CTkButton(lateral, text="← Voltar ao início", fg_color=RED, hover_color=RED_DARK, command=self.fechar).pack(side="bottom", fill="x", padx=22, pady=24)

        principal = ctk.CTkScrollableFrame(self, fg_color=BG)
        principal.grid(row=0, column=1, sticky="nsew", padx=30, pady=25)
        ctk.CTkLabel(principal, text="Preferências", font=("Arial", 28, "bold"), text_color=TEXT).pack(anchor="w", pady=(0, 4))
        ctk.CTkLabel(principal, text="Ajuste o comportamento do aplicativo sem alterar o código.", text_color=MUTED).pack(anchor="w", pady=(0, 20))

        interface = self.card(principal, "Interface")
        self.escala = ctk.StringVar(value=f"{int(float(self.dados['escala_interface']) * 100)}%")
        self.linha_opcao(interface, "Escala da interface", "Tamanho geral dos componentes", lambda pai: ctk.CTkOptionMenu(pai, values=["90%", "100%", "110%", "120%"], variable=self.escala))
        self.animacoes = ctk.BooleanVar(value=bool(self.dados["animacoes"]))
        self.linha_opcao(interface, "Animações", "Ativa ou desativa o movimento do menu principal", lambda pai: ctk.CTkSwitch(pai, text="", variable=self.animacoes))
        self.maximizado = ctk.BooleanVar(value=bool(self.dados["abrir_maximizado"]))
        self.linha_opcao(interface, "Abrir maximizado", "Telas principais iniciam ocupando a área disponível", lambda pai: ctk.CTkSwitch(pai, text="", variable=self.maximizado))

        seguranca = self.card(principal, "Ferramentas de rede")
        self.max_portas = ctk.StringVar(value=str(self.dados["max_portas_scan"]))
        self.linha_opcao(seguranca, "Limite do scanner", "Quantidade máxima de portas por varredura", lambda pai: ctk.CTkOptionMenu(pai, values=["32", "64", "128", "256"], variable=self.max_portas))
        self.timeout = ctk.StringVar(value=str(self.dados["timeout_porta"]))
        self.linha_opcao(seguranca, "Timeout por porta", "Tempo máximo de conexão em segundos", lambda pai: ctk.CTkOptionMenu(pai, values=["0.10", "0.25", "0.50", "1.00"], variable=self.timeout))

        sobre = self.card(principal, "Sobre")
        ctk.CTkLabel(sobre, text="CiberToolBox v0.3.0\nProjeto acadêmico de cibersegurança com foco em centralização, usabilidade e aprendizado.", justify="left", text_color=MUTED, wraplength=680).pack(anchor="w", padx=22, pady=(0, 18))

        botoes = ctk.CTkFrame(principal, fg_color="transparent")
        botoes.pack(fill="x", pady=12)
        ctk.CTkButton(botoes, text="Salvar configurações", height=44, fg_color=RED, hover_color=RED_DARK, command=self.salvar).pack(side="left")
        ctk.CTkButton(botoes, text="Restaurar padrões", height=44, fg_color="transparent", border_width=1, border_color=BORDER, command=self.restaurar).pack(side="left", padx=12)
        self.status = ctk.CTkLabel(principal, text="", text_color=GREEN)
        self.status.pack(anchor="w")

    def card(self, pai, titulo):
        frame = ctk.CTkFrame(pai, fg_color=CARD, border_width=1, border_color=BORDER, corner_radius=14)
        frame.pack(fill="x", pady=10)
        ctk.CTkLabel(frame, text=titulo, font=("Arial", 18, "bold"), text_color=ORANGE).pack(anchor="w", padx=22, pady=(18, 12))
>>>>>>> 9c2c9652a37a693c9903b92e712abd3019443202
        return frame
...
=======
        ctk.CTkLabel(texto, text=titulo, font=("Arial", 14, "bold"), text_color=TEXT).pack(anchor="w")
        ctk.CTkLabel(texto, text=descricao, text_color=MUTED).pack(anchor="w")
>>>>>>> 9c2c9652a37a693c9903b92e712abd3019443202
...
=======
        self.status.configure(text="Configurações salvas com sucesso.")

    def restaurar(self):
        if not messagebox.askyesno("Restaurar", "Deseja restaurar as configurações padrão?", parent=self):
            return
>>>>>>> 9c2c9652a37a693c9903b92e712abd3019443202
...
=======
        if self.janela_principal.winfo_exists():
            ...
            config = self.model.carregar()
            try:
                self.janela_principal.state("zoomed" if config.get("abrir_maximizado", True) else "normal")
            except ctk.TclError:
                pass
            self.janela_principal.lift()
            self.janela_principal.focus_force()
